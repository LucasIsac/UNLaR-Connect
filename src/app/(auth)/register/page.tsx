...
function RegisterForm() {
  ...
  const getPasswordStrength = (pass: string) => {
    ...
    if (score <= 1) {
      ...
      return { 
        score: 2, 
        label: "Media / Intermedia 🟡", 
        colorClass: "bg-[#FBBC05]", 
        textClass: "text-[#FBBC05]" 
      };
    } else {
      return { 
        score: 3, 
        label: "Fuerte / Muy Segura 🟢", 
  ...
  const handleEmailRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        router.push(`/login?registered=true&next=${encodeURIComponent(nextParam)}`);
      } else {
        setErrorMsg(result.error ?? "No se pudo completar el registro. Intentalo de nuevo.");
        setLoading(false);
      }
    } catch (err) {
      console.error(err);
      setErrorMsg("Hubo un error inesperado al procesar tu registro.");
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-[#1C1917]/70 backdrop-blur-xl border border-[#F59E0B]/10 rounded-lg p-8 shadow-[0_8px_32px_rgba(12,10,9,0.5)]">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <span className="text-xs font-sans text-[#d8c3ad]/50 uppercase tracking-widest">O completá tus datos</span>
        <div className="flex-grow h-[1px] bg-[#534434]/30" />
      </div>
          ...
          <div>
            ...
            <div className="relative">
              <span className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <User className="h-4 w-4 text-[#a08e7a]/60" />
              </span>
              <input
                type="text"
                disabled={loading}
                ...
                className="block w-full pl-9 pr-3 py-3 bg-[#0C0A09]/60 border border-[#534434]/40 rounded-xl text-sm font-sans text-[#e8e1df] placeholder-[#a08e7a]/50 focus:outline-none focus:border-[#F59E0B] focus:ring-1 focus:ring-[#F59E0B] disabled:opacity-50"
              />
            </div>
          </div>
          ...
          {password && (
            <div className="mt-2 space-y-1.5 animate-fade-in">
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-sans font-semibold text-[#d8c3ad]/60 uppercase tracking-wider">
                  Seguridad de la clave
                </span>
                <span className={`text-[10px] font-sans font-bold ${strength.textClass}`}>
                  {strength.label}
                </span>
              </div>
              <div className="flex gap-1 h-1 w-full bg-[#0C0A09]/80 rounded-full overflow-hidden p-[1px] border border-[#534434]/20">
                <div className={`h-full rounded-full transition-all duration-500 ${strength.score >= 1 ? strength.colorClass : 'bg-transparent'}`} style={{ width: '33.33%' }} />
                <div className={`h-full rounded-full transition-all duration-500 ${strength.score >= 2 ? strength.colorClass : 'bg-transparent'}`} style={{ width: '33.33%' }} />
                <div className={`h-full rounded-full transition-all duration-500 ${strength.score >= 3 ? strength.colorClass : 'bg-transparent'}`} style={{ width: '33.33%' }} />
              </div>
            </div>
          )}
        </div>
...
export default function RegisterPage() {
  return (
    <main className="min-h-screen flex items-center justify-center bg-[#0C0A09] relative overflow-hidden px-4 py-12">
      {/* Background Ambience Glows */}
      <div className="absolute top-[-20%] right-[-10%] w-[50%] h-[50%] rounded-full bg-[#F59E0B]/10 blur-[120px] pointer-events-none" />
      <div className="absolute bottom-[-20%] left-[-10%] w-[50%] h-[50%] rounded-full bg-[#E2775F]/10 blur-[120px] pointer-events-none" />
      ...
      <div className="w-full max-w-md relative z-10 animate-fade-in">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="font-sans text-sm text-[#d8c3ad]/80 mt-1 text-center">
            Unite a la comunidad estudiantil. ¡Registrate al toque!
          </p>
        </div>
