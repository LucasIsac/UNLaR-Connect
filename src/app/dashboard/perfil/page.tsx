...
export default function PerfilPage() {
  ...
  const [tutorSubjects, setTutorSubjects] = useState<DbSubject[]>([]);
  ...
  const [editCareerId, setEditCareerId] = useState(1);
  ...
  // Load all profile details on mount
  useEffect(() => {
    async function loadData() {
      try {
        const [profileData, subjectsData, availabilityData, careersData, officialSubjects] = await Promise.all([
          fetchUserProfile(),
          fetchTutorSubjects(),
          fetchTutorAvailability(),
          fetchCareers(),
        ...
        setAllSubjects(officialSubjects);
