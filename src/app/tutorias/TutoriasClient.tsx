...
export default function TutoriasClient({ currentUser, initialHeaderData }: TutoriasClientProps) {
  ...
  const myTutorData = allTutors.find((t) => t.id === currentUser.id);
  ...
  const handleCreateOpenCall = async () => {
    setIsRequesting(true);
    setPendingCallMessage("Creando sala abierta...");
    
    const res = await createOpenCall();
    if (res.success && res.data) {
          ...
          {isTutor && (
            <div className="flex items-center gap-3">
              <button
                ...
                className="bg-accent/10 hover:bg-accent/20 border border-accent/20 text-accent font-bold px-3 py-1.5 rounded-xl transition-all text-xs flex items-center gap-1.5 whitespace-nowrap"
              >
